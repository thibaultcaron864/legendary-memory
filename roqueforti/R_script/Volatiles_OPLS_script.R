...
#Y (object to predict)
pls.y<-matrix(as.numeric(vol_cheese_gp[,1]),ncol=1)
...
#visualize predictive error for trainning data (RMSEP): root mean squared error of prediction on the training data (RMSEP)
plot.OSC.results(mods,plot="RMSEP",groups=groups)

# based on this we can see that 3 components minimize the error for both 0 or 1 OSC models
# recalculate OSC model using 1 OSC and 2 total components ( 1 non - OSC)
mods<-OSC.correction(pls.y,pls.data,comp=3,OSC.comp=1,validation = "LOO",methods="oscorespls")
...
plot.OSC.results(mods,plot="scores",groups=pls.y)
