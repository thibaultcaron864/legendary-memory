...
tab<-read.csv(file="./INDEL_pour_R_lipolysis.csv", sep=";")
cross<-read.cross("csv", file = "./INDEL_pour_R_lipolysis.csv", sep = ";", na.strings = "-", genotypes = c("RR","RC","CC")) #cr?ation du croisement g?n?tique
...
par(mfrow=c(3,2))
for(i in 1:6){
  plot(scal.out.hk, scal.out.imp, scal.out.em, chr=i, lty=1, col=c("blue", "red", "black"),ylim=c(0,60))
}
